...
class LoopBlock:
        ...
        if self.dimension == 0:
            for aloop in self.loops:
                for apacket in aloop.recursive_iter():
                    ...
                    yield dict(top_items + apacket.items())
        else:
                ...
                if self.loops:
                  for aloop in self.loops:
                    for apacket in aloop.recursive_iter(coord=coord+[i]):
                        ...
                        yield dict(kvpairs + apacket.items())
    ...
    def AddLoopItem(self,data,precheck=False,maxlength=-1):
        ...
        if isinstance(data[0],(TupleType,ListType)):
           ...
           if data[1] == [] or self.get_dim(data[1])[0] == self.dimension:
               if not precheck:
               ...
               if self.dimension > 0:
                   if self.no_packets <= 0:
                       self.no_packets = len(data[1])  #first item
               ...
               raise StarLengthError, "input data dim %d != required dim %d: %s %s" % (self.get_dim(data[1])[0],self.dimension,data[0],`data[1]`)
    ...
    def get_dim(self,dataitem,current=0,packlen=0):
        ...
        #    print "Get_dim: %d: %s" % (current,`dataitem`)
            return self.get_dim(dataitem[0],current+1,len(dataitem))
    ...
    def ChangeItemOrder(self,itemname,newpos):
        import string
        def low_case(item):       #need to skip Loop blocks
            try:
        ...
        try:
            ...
            testname = itemname
        testpos = lowcase_order.index(testname)
    ...
    def format_packet_item(self,pack_item,indent):
        ...
        curstring = ''
        if isinstance(pack_item,(StringType,IntType,FloatType,LongType)):
...
def ReadStar(filename,maxlength=2048,dest=StarFile(),scantype='standard'):
    import YappsStarParser,string
    ...
    try: 
        if scantype == 'standard':
            parser = YappsStarParser.StarParser(YappsStarParser.StarParserScanner(text))
        else:
            parser = YappsStarParser.StarParser(YappsStarParser.Scanner(None,[],text,scantype='flex'))
        proto_star = getattr(parser,"input")()
    except YappsStarParser.SyntaxError:
        errorstring = 'Syntax error in input file: last value parsed was %s' % YappsStarParser.lastval
